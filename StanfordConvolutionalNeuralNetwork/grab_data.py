...
def rand_select(images):
	...
	patches = np.zeros((global_num_patches, global_patch_dim ** 2)) # (10000, 100)
	...
	# Flatten the images
	patches = np.reshape(patches, (len(patches), len(patches[0].ravel())))
	return patches


####### Code #######
# We need to grab 10000 (10, 10) patches from the MNIST dataset at random to use for our sparse autoencoder
