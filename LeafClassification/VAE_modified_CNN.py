...
def plot_results(models,
	...
	plt.ylim(-8, 15)
    	...
    	grid_x = np.linspace(-8, 4, n)
    	grid_y = np.linspace(-12, 2, n)[::-1]
...
tr_train = tr_train
tr_val = tr_val
te_test = test_mod_list

# network parameters
input_shape = (global_max_dim, global_max_dim, 1)
...
x = Conv2D(1, kernel_size = (5, 5), activation = 'relu', padding = 'same')(inputs)
x = Dropout(0.2)(x)
x = Conv2D(16, kernel_size = (5, 5), activation = 'relu', padding = 'same')(x)
x = Dropout(0.2)(x)
x = Conv2D(8, kernel_size = (5, 5), activation = 'relu', padding = 'same')(x)
x = Flatten()(x)
...
x = Dense(global_max_dim * global_max_dim * 8, activation = 'relu')(x)
x = Reshape((global_max_dim, global_max_dim, 8))(x)
x = Conv2DTranspose(8, kernel_size = (5, 5), activation = 'relu', padding = 'same')(x)
x = Dropout(0.2)(x)
x = Conv2DTranspose(16, kernel_size = (5, 5), activation = 'relu', padding = 'same')(x)
x = Dropout(0.2)(x)
x = Conv2DTranspose(1, kernel_size = (5, 5), activation = 'sigmoid', padding = 'same')(x)
outputs =x
...
reconstruction_loss = mse(Flatten()(inputs), Flatten()(outputs)) * global_max_dim * global_max_dim
...
model_file = 'VAE/vae_mlp_leaves_weights_nn_dim50_CNN.h5'
if args.load_model:
