...
ordered_vals = rand_sample(x_vals, y_vals_train)


# Split the x and y arrays
x_ordered = ordered_vals[:, :-10]
y_ordered = ordered_vals[:, 401:411]
m = len(x_ordered) 

initial_cost = reg_cost(theta_vals, x_ordered, y_ordered, lambda1)
print "Our initial cost value is %g." %(initial_cost)
	
# Use scipys minimize function to compute the theta values
minimum = scipy.optimize.minimize(fun = reg_cost, x0 = theta_vals, method = 'CG', tol = 1e-4, jac = backprop, args = (x_ordered, y_ordered, lambda1))#, options = {'disp': True}

# Set up the new theta values we hav from our minimize function
theta_new = minimum.x

final_cost = reg_cost(theta_new, x_ordered, y_ordered, lambda1)
