...
# Let's ranomize x and y
random_vals = rand_sample(x_vals, y_vals_train)

# Split the x and y arrays
x_val = random_vals[:, :-10]
y_val = random_vals[:, 401:411]

initial_cost = reg_cost(theta_vals, x_vals, y_vals, lambda1)
...
final_cost = reg_cost(theta_new, x_vals, y_vals, lambda1)
print "Our final cost value is %g." %(final_cost)

# Save the theta values to use later
np.savetxt('finalThetasRandom5000.out', theta_new, delimiter = ',')
